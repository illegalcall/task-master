...
// Updated MockProducer that implements sarama.SyncProducer.
...
func (p *MockProducer) AbortTxn() error {
	// Dummy implementation for transaction abort.
	return nil
}

// Add these methods to MockProducer
...
func (p *MockProducer) AddOffsetsToTxn(map[string][]*sarama.PartitionOffsetMetadata, string) error {
	// Dummy implementation for adding offsets to a transaction.
	return nil
}
...
func TestLogin(t *testing.T) {
	...
	// (Optional) You can also check that the token expires in about 72 hours.
	expClaim := time.Now().Add(72 * time.Hour).Unix()
	tokenStr := token.(string)
	claims := jwt.MapClaims{}
	jwt.ParseWithClaims(tokenStr, claims, func(token *jwt.Token) (interface{}, error) {
		return []byte("testsecret"), nil
	})
	assert.Equal(t, expClaim, int64(claims["exp"].(float64)))
